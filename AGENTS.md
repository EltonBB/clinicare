...
- **Compliance:** **GDPR is the active regime** (Kosovo's GDPR-aligned law + any EU clinic). App-level safeguards (audit logging, auto-logoff, role-based/minimum-necessary access) are built **now, alongside features, not retrofitted later** — they satisfy GDPR today and keep the HIPAA path (ROADMAP.md §5, kept ready for if the US reopens) cheap to resume.
...
- **Channel plan** (ROADMAP.md): **Baileys WhatsApp is the only wired channel today** (Kosovo-only, disposable, isolated) — confirmed sending and receiving real messages 2026-08-29. Resend (email) and Twilio (SMS) are planned adapters, not yet built. The official WhatsApp upgrade (once a US entity + Meta access exist) is a later step, paused alongside the US market rather than actively gated; all channels stay behind the messaging seam so a provider can be swapped if needed.
