...
- Updated the shipped public Vela icon asset from the uploaded `vela_icon_new_gradient.svg` so the logo mark now uses the new cobalt/neon blue to light blue gradient (`#0A22FF` to `#64B6FF`). Verified the SVG asset, then reran `npm run lint` and `npm run build`.
