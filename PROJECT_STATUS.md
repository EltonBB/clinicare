...
- Completed the public marketing visibility and content follow-up. Fixed the Vela logo tile contrast, no-wrap CTA button sizing, icon tile spacing, global brand font stack, Pricing page product explanation panel, and About page context sections. Verified with `npm run lint`, `npm run build`, and Playwright desktop/mobile checks for `/`, `/pricing`, and `/about`; the local preview server is running again on `http://localhost:3000/`.
