...
- Completed the Cal.com-inspired marketing redesign pass using the supplied Vela brand-scene images only. The public site now has a cleaner centered hero, large visual product proof, scheduling/client/report image sections, workflow steps, refined Product/Pricing/About/Contact pages, stronger CTAs, and better mobile typography. Verified with `npm run lint`, `npm run build`, and Chrome headless desktop/mobile screenshots. Deployment should continue through GitHub push only, not direct Vercel CLI deploy.
